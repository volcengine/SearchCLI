...
4. For persistent changes to an existing scene, run `vs recommend scene get --application-id <application-id> --scene-id <scene-id>` first and inspect the current scene before mutating it. Treat the readback as the base payload for a full publish; first-level `Config` sections may be replaced, but nested field edits require a full updated first-level section.
