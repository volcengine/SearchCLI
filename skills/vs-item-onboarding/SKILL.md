...
3. `item plan --file <data> --type <item|video> --goal "<goal>" --schema-source console` — generate plan directory through the stable schema workflow: the CLI handles the internal upload/signature and schema-inference steps, then continues plan generation. Add `--skip-app` when the requested mode is `dataset-only`. If execution later goes through `item provision` or `item apply`, those commands also accept `--skip-app` as an execution-time guard rail.
