...
| `Config.PerDatasetConfigs[].ShuffleConfig.Rules[].WindowSize` | `> 0` and `>= MaxSize` | Window must be no smaller than the max display size. |
| `Config.PerDatasetConfigs[].ShuffleConfig.Rules[].MaxSize` | effective value `> 0` | Maximum number displayed within the window. |
