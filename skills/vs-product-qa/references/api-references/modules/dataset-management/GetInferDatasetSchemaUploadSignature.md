# GetInferDatasetSchemaUploadSignature

## 接口概览

- 模块分类：Dataset Management
- Service：DashboardService
- RPC：GetInferDatasetSchemaUploadSignature
- HTTP Method：`POST`
- Request Path：`/api/v1/GetInferDatasetSchemaUploadSignature`
- Request Type：`dataset.GetInferDatasetSchemaUploadSignatureReq`
- Response Type：`dataset.GetInferDatasetSchemaUploadSignatureResp`
- Top Action：GetInferDatasetSchemaUploadSignature
- Top Version：2025-03-01
- Service Path：dashboard_service
- 源定义：`console/idl/handler.proto:273`
...
| --- | --- | --- | --- | --- | --- |
| FileName | string | 否 | body | 必填，本地待上传文件名，仅支持 jsonl | - |
...
| FileKey | string | 否 | body | TOS object key | - |
| TosBucket | string | 否 | body | TOS bucket | - |
| HttpMethod | string | 否 | body | HTTP method，当前固定为 PUT | - |
| ExpiresInSeconds | integer | 否 | body | 签名有效期（秒） | - |
