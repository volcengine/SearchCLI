...
type FeatureId = "search" | "recommend" | "chat";
...
export function App() {
  ...
  return (
    <main className="h-svh overflow-hidden bg-background px-4 py-8 text-foreground sm:px-8">
      <div
        className={cn(
          "mx-auto flex min-h-0 w-full max-w-160 flex-col gap-3",
          useFullHeightLayout && "h-full"
        )}
      >
...
async function postJson<TResponse>(path: string, body?: unknown): Promise<TResponse> {
  ...
  if (!response.ok) {
    throw new Error(getResponseErrorMessage(data, `请求失败，状态码 ${response.status}。`));
  }
...
async function streamChat({
  ...
  try {
    while (true) {
      ...
      const completed = handleSseBuffer(buffer, event => {
        handleChatSseEvent(event, onSession, onChunk);
      });
      buffer = completed.remaining;
      if (completed.done) {
        receivedDone = true;
        await reader.cancel();
        return;
...
function handleChatSseEvent(
  ...
  if (event.event === "error") {
    throw new Error(
      getResponseErrorMessage(event.data as { error?: string }, "对话请求失败。")
    );
  }
}
...
function getResponseErrorMessage(data: { error?: string }, fallback: string): string {
