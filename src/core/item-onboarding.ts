...
function buildValidationResult(
  ...
  if (datasetType === 'item') {
    validateParentVariantRows(fields, records, primaryKeyField, issues);
  }
...
function validateParentVariantRows(
  fields: ItemFieldProfile[],
  records: Array<Record<string, unknown>>,
  primaryKeyField: string,
  issues: ItemValidationIssue[]
): void {
  const parentFields = detectParentVariantFields(fields);
  if (!parentFields.itemTypeField && !parentFields.parentIdField) {
    return;
  }
  if (!parentFields.itemTypeField || !parentFields.parentIdField) {
    issues.push({
      severity: 'error',
      code: 'parent_variant_attr_incomplete',
      field: parentFields.itemTypeField ?? parentFields.parentIdField,
      detail: 'item_type and parent_id must be configured together for parent/variant item modeling.'
    });
    return;
  }

  const itemTypeByPrimaryKey = new Map<string, string>();
  for (const record of records) {
    const pk = normalizeDisplayText(getValueAtPath(record, primaryKeyField));
    const itemType = normalizeItemTypeValue(getValueAtPath(record, parentFields.itemTypeField));
    if (pk) {
      itemTypeByPrimaryKey.set(pk, itemType);
    }
  }

  let invalidItemTypeCount = 0;
  let variantMissingParentCount = 0;
  let parentWithParentIDCount = 0;
  let parentIDNotParentCount = 0;
  let parentIDNotUploadedCount = 0;
  const invalidExamples: unknown[] = [];
  const missingParentExamples: unknown[] = [];
  const parentWithParentExamples: unknown[] = [];
  const wrongParentExamples: unknown[] = [];
  const missingParentUploadExamples: unknown[] = [];

  for (const record of records) {
    const itemTypeRaw = getValueAtPath(record, parentFields.itemTypeField);
    const parentIDRaw = getValueAtPath(record, parentFields.parentIdField);
    const itemType = normalizeItemTypeValue(itemTypeRaw);
    const parentID = normalizeDisplayText(parentIDRaw);
    const pk = normalizeDisplayText(getValueAtPath(record, primaryKeyField));

    if (itemType && itemType !== 'parent' && itemType !== 'variant') {
      invalidItemTypeCount += 1;
      if (invalidExamples.length < 3) invalidExamples.push(itemTypeRaw);
      continue;
    }
    if (itemType === 'variant' && !parentID) {
      variantMissingParentCount += 1;
      if (missingParentExamples.length < 3) missingParentExamples.push(pk || record);
      continue;
    }
    if (itemType === 'parent' && parentID) {
      parentWithParentIDCount += 1;
      if (parentWithParentExamples.length < 3) parentWithParentExamples.push(pk || record);
      continue;
    }
    if (itemType === 'variant' && parentID) {
      const parentType = itemTypeByPrimaryKey.get(parentID);
      if (parentType === undefined) {
        parentIDNotUploadedCount += 1;
        if (missingParentUploadExamples.length < 3) missingParentUploadExamples.push(parentID);
        continue;
      }
      if (parentType !== 'parent') {
        parentIDNotParentCount += 1;
        if (wrongParentExamples.length < 3) wrongParentExamples.push(parentID);
      }
    }
  }

  if (invalidItemTypeCount > 0) {
    issues.push({
      severity: 'error',
      code: 'invalid_item_type_value',
      field: parentFields.itemTypeField,
      detail: `item_type must be empty, "parent", or "variant"; ${invalidItemTypeCount} records contain invalid values.`,
      affectedCount: invalidItemTypeCount,
      examples: invalidExamples
    });
  }
  if (variantMissingParentCount > 0) {
    issues.push({
      severity: 'error',
      code: 'variant_parent_id_missing',
      field: parentFields.parentIdField,
      detail: `variant items must provide parent_id; ${variantMissingParentCount} records are missing it.`,
      affectedCount: variantMissingParentCount,
      examples: missingParentExamples
    });
  }
  if (parentWithParentIDCount > 0) {
    issues.push({
      severity: 'error',
      code: 'parent_item_parent_id_not_empty',
      field: parentFields.parentIdField,
      detail: `parent items must not provide parent_id; ${parentWithParentIDCount} records contain one.`,
      affectedCount: parentWithParentIDCount,
      examples: parentWithParentExamples
    });
  }
  if (parentIDNotParentCount > 0) {
    issues.push({
      severity: 'error',
      code: 'parent_id_must_be_parent',
      field: parentFields.parentIdField,
      detail: `${parentIDNotParentCount} parent_id values point to records whose item_type is not "parent".`,
      affectedCount: parentIDNotParentCount,
      examples: wrongParentExamples
    });
  }
  if (parentIDNotUploadedCount > 0) {
    issues.push({
      severity: 'warning',
      code: 'parent_item_not_uploaded',
      field: parentFields.parentIdField,
      detail: `${parentIDNotUploadedCount} parent_id values are not present in this input file. Ensure parent items already exist or are imported first.`,
      affectedCount: parentIDNotUploadedCount,
      examples: missingParentUploadExamples
    });
  }
}
...
function lastPathToken(pathValue: string): string {
  const parts = pathValue.split('.');
  return (parts[parts.length - 1] ?? pathValue).toLowerCase();
}

function getValueAtPath(record: Record<string, unknown>, pathValue: string): unknown {
...
function pickItemTypeField(attrFields: Record<string, string[]> | undefined, allowed: Set<string>): string | undefined {
  return ['ImageItemType', 'multi_modal_item_type']
    .flatMap(attrName => attrFields?.[attrName] ?? [])
...
function resolveSchemaAttrOverrides(
  ...
  if (datasetType === 'item') {
    if (hasAttr('ImageItemType')) {
      return { BizAttr: 20, Type: 1 };
    }
    if (hasAttr('ImageParentId')) {
      return { BizAttr: 201, Type: 1 };
    }
  }
  if (datasetType === 'video') {
...
function inferFieldDescription(
  ...
  if (['parent_id', 'parent_item_id'].includes(field.name)) {
    return 'Parent item identifier for variant items.';
  }
  if (inferredMeaning && inferredMeaning.trim().length > 0) {
...
function buildSearchSceneUpdateArtifact(
  defaults: ItemPlanFile['defaults'],
  itemTypeField: string | undefined,
  itemTypeResult: ItemTypeResultMode
): Record<string, unknown> {
  const itemTypeFilterConfig = buildItemTypeFilterConfig(itemTypeField, itemTypeResult);
  return {
    ...
    Config: {
      SearchConfig: {
        RetrieveConfigs: itemTypeFilterConfig ? [{ FilterConfig: { ItemTypeFilter: itemTypeFilterConfig } }] : []
      }
    }
  };
}

function buildRecommendSceneUpdateArtifact(
  defaults: ItemPlanFile['defaults'],
  itemTypeField: string | undefined,
  itemTypeResult: ItemTypeResultMode
): Record<string, unknown> {
