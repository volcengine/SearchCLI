...
export default class DatasetSubscriptionGet extends Command {
  static override description = 'Get a dataset data source subscription.';

  static override examples = [
    '<%= config.bin %> dataset subscription get --task-id task_123',
    '<%= config.bin %> dataset subscription get --data @get-subscription.json'
  ];
