...
export async function runItemApplyCommand(options: ItemApplyCommandOptions): Promise<void> {
  ...
  if (phase === 'verify') {
    await runItemVerifyCommand({
      ...
      recommendParentIds: options.recommendParentIds,
...
async function executeItemVerify(options: ItemVerifyCommandOptions): Promise<Record<string, unknown>> {
  ...
  if (recommendBhvSceneTypes.length > 0) {
    ...
    const parentItems = buildRecommendParentItems(options.recommendParentId, options.recommendParentIds);
    if (options.recommendUserId || parentItems) {
      ...
      steps.push({
        ...
        ok: true,
        skipped: true,
        detail: `Recommend scene ${recommendSceneId} is ready. Pass --recommend-user-id, --recommend-parent-id, or --recommend-parent-ids to run runtime recommend smoke.`
      });
    }
  } else {
    steps.push({
...
function formatSchemaBizAttr(value: unknown): string | undefined {
  switch (value) {
    ...
    case 94:
      return 'MultiModalParentId';
    case 103:
      return 'MultiModalItemType';
    case 201:
