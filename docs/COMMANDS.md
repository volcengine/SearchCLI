...
*   `vs search scene create --application-id <id> --name <name>`
    *   Usage: `vs search scene create --application-id <id> --name <name> [--description <text>] [--search-config @search.json] [--item-type-result variant|parent --item-dataset-id <id>] [--item-type-field item_type] [service flags]`
    *   Usage: `vs search scene create --application-id <id> --data @payload.json [service flags]`
    *   Description: create a new search scene under the target application; `--item-type-result` writes `Config.PerDatasetConfigs[].FilterConfig.ItemTypeFilter` for parent/variant item datasets
    *   Key flags: `--application-id`, `--name`, `--description`, `--search-config`, `--item-dataset-id`, `--item-type-result`, `--item-type-field`, `--data`
    *   Examples: `vs search scene create --application-id 123 --name "default-search"`; `vs search scene create --application-id 123 --name "parent-search" --search-config @search.json --item-dataset-id ds_123 --item-type-result parent`; `vs search scene create --application-id 123 --data @payload.json`
